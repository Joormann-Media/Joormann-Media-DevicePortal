...
EMIT_REPO_DIR="$(resolve_repo_dir "${PLAYER_REPO_REF}" "${SERVICE_USER}" "${INSTALL_DIR}" "${PORTAL_REPO_DIR}")"
EMIT_INSTALL_DIR="${INSTALL_DIR}"
if [[ -z "${EMIT_INSTALL_DIR}" ]]; then
  EMIT_INSTALL_DIR="${EMIT_REPO_DIR}"
fi
emit "install_dir" "${EMIT_INSTALL_DIR}"
emit "use_service" "${USE_SERVICE}"
emit "autostart" "${AUTOSTART}"
emit "repo_dir" "${EMIT_REPO_DIR}"
